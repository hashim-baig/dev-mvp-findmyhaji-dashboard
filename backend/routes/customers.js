...
// POST /api/customers/login - Post login
// Validation rules
...
router.post('/login', loginValidation, async (req, res) => {
  try {
    ...
    const role =  3;//role id for customers
    // Find admin user
    const user = await User.login({ email, role });
    if (!user) {
    ...
    // 🔹 Check is customer is active
    if (!user.status) {
        return res.status(404).json({
            success: false,
            error: 'Customer is not active.Please contact to admin!' 
        });
    }
    ...
    const role_result = Role.findOne({id:user.role});
    const role_data = role_result;
    res.json({
      status:'success',
